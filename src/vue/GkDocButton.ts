...
function fileIconSvg(fileType?: DocItem['fileType']): string {
  ...
  const label =
    fileType === 'pdf'
      ? `<text x="3.5" y="11" font-size="3.5" font-weight="bold" fill="${color}" font-family="sans-serif">PDF</text>`
      : fileType === 'docx'
        ? `<text x="2.5" y="11" font-size="3" font-weight="bold" fill="${color}" font-family="sans-serif">DOCX</text>`
        : fileType === 'xlsx'
          ? `<text x="2.5" y="11" font-size="3" font-weight="bold" fill="${color}" font-family="sans-serif">XLSX</text>`
          : '';
          ...
          return h(
            ...
            [
              ...
              doc.size &&
                h(
                  'span',
                  { style: { color: 'var(--gk-text-muted, #6b7280)', fontSize: '12px' } },
                  `(${doc.size})`,
                ),
            ],
          );
      ...
      return h('div', { style: { display: 'inline-block' } }, [
        h(
          'button',
          {
            ...
            onClick: (e: Event) => {
              e.stopPropagation();
              isOpen.value ? (isOpen.value = false) : openDropdown();
            },
            style: btnStyle,
          },
          [DownloadIcon(), label, ChevronIcon()],
        ),
        h(Teleport, { to: 'body' }, [
          isOpen.value &&
            h(
