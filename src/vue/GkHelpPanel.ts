...
function typeIconSvg(type: HelpItemType): ReturnType<typeof h> {
  ...
  switch (type) {
    ...
    case 'video':
      return h('svg', { ...svgProps, fill: 'currentColor' }, [h('path', { d: 'M8 5v14l11-7z' })]);
    default:
      return h('svg', svgProps, [
        h('polygon', {
          points:
...
export const GkHelpPanel = defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    function onClickOutside(e: MouseEvent) {
      ...
      if (!panelRef.value?.contains(t) && !btnRef.value?.contains(t)) {
        isOpen.value = false;
      }
    }
    ...
    return () => {
      ...
      const panelNode = h(
        ...
        [
          ...
          h(
            ...
            [
              h(
                'span',
                {
                  style: { fontWeight: '600', fontSize: '13px', color: 'var(--gk-text, #1f2937)' },
                },
                'Available help',
              ),
              h(
                'span',
                { style: { fontSize: '12px', color: 'var(--gk-text-muted, #6b7280)' } },
          ...
          h(
            'div',
            { style: { padding: '8px 16px', borderTop: '1px solid var(--gk-border, #e5e7eb)' } },
            [
              h(
                'button',
                {
