import { defineComponent, h, ref, watch, onMounted, onUnmounted, nextTick, Teleport } from 'vue';
...
export const GkTour = defineComponent({
  ...
  setup(props, { emit, expose }) {
    ...
    async function showStep(index: number) {
      ...
      OverlayManager.show(
        targetEl,
        props.spotlightPadding as number,
        props.overlayOpacity as number,
      );
      ...
      return h(Teleport, { to: 'body' }, [
        h('div', { ref: popoverEl, style: popoverStyle }, [
          ...
          step.title &&
            h(
              'div',
              { style: { fontWeight: '600', fontSize: '15px', marginBottom: '6px' } },
              step.title,
            ),
          // Content
          h(
