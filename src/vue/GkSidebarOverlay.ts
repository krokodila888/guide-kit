...
function renderTextSection(section: SidebarSection) {
  return h(
    'div',
    { style: { fontSize: '14px', lineHeight: '1.6', color: 'var(--gk-text, #1f2937)' } },
    section.content ?? '',
  );
}
        ...
        [
          h('input', {
            ...
            style: {
              width: '16px',
              height: '16px',
              accentColor: 'var(--gk-primary, #2563eb)',
              flexShrink: '0',
            },
          }),
          h(
            ...
            {
              style: {
                color: checked ? 'var(--gk-text-muted, #6b7280)' : 'var(--gk-text, #1f2937)',
    ...
    return () => {
      ...
      const headerNode = h(
        ...
        [
          h(
            'span',
            { style: { fontWeight: '600', fontSize: '15px', color: 'var(--gk-text, #1f2937)' } },
      ...
      const sectionNodes = sections.map((section, i) => {
        ...
        const headerEl = section.title
          ? h(
              'div',
              {
                onClick: isCollapsible
                  ? () => {
                      collapsedState[i] = !isCollapsed;
                    }
                  : undefined,
                style: {
                  display: 'flex',
        ...
        if (!isCollapsed) {
          switch (section.type) {
            ...
            case 'steps':
              ...
              break;
            case 'checklist':
              bodyEl = renderChecklistSection(section, checkedState);
              break;
            case 'formula':
