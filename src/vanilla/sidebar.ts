...
import type { VanillaSidebarOptions, VanillaSidebarSection, SidebarInstance } from './index.types';
...
function renderChecklistSection(section: VanillaSidebarSection): HTMLElement {
  ...
  const checkboxStates = new Map<string, boolean>(items.map(it => [it.id, it.checked ?? false]));

  items.forEach(item => {
    const row = document.createElement('label');
    row.style.cssText =
      'display:flex;align-items:center;gap:10px;cursor:pointer;margin-bottom:8px;font-size:14px';

    const cb = document.createElement('input');
    cb.type = 'checkbox';
    cb.checked = item.checked ?? false;
    cb.style.cssText =
      'width:16px;height:16px;cursor:pointer;accent-color:var(--gk-primary,#2563eb)';
    ...
    cb.addEventListener('change', () => {
      ...
      labelText.style.color = cb.checked
        ? 'var(--gk-text-muted,#6b7280)'
        : 'var(--gk-text,#1f2937)';
...
function buildSectionWrapper(section: VanillaSidebarSection): HTMLElement {
  ...
  switch (section.type) {
    case 'text':
      bodyEl = renderTextSection(section);
      break;
    case 'steps':
      bodyEl = renderStepsSection(section);
      break;
    case 'checklist':
      bodyEl = renderChecklistSection(section);
      break;
    case 'formula':
      bodyEl = renderFormulaSection(section);
      break;
    case 'doc':
      bodyEl = renderDocSection(section);
      break;
    case 'video':
      bodyEl = renderVideoSection(section);
      break;
    default:
