...
export function createHelpPanel(options: VanillaHelpPanelOptions): HelpPanelInstance {
  ...
  function buildPanel(): HTMLElement {
    ...
    if (items.length === 0) {
      ...
      empty.style.cssText =
        'padding:20px 16px;text-align:center;color:var(--gk-text-muted,#6b7280);font-size:13px';
      ...
      items.forEach(item => {
        ...
        row.addEventListener('mouseenter', () => {
          if (enabled) row.style.background = 'var(--gk-border, #f3f4f6)';
        });
  ...
  const onClickOutside = (e: PointerEvent) => {
    if (panelEl && !panelEl.contains(e.target as Node) && !triggerBtn.contains(e.target as Node)) {
      close();
    }
  };
